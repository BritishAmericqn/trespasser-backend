        ...
        if (now - weapon.lastFireTime >= fireInterval) {
          ...
          if (fireResult.success) {
            ...
            for (const event of fireResult.events) {
              ...
              if (event.type === EVENTS.PROJECTILE_CREATED || 
                  event.type === EVENTS.PROJECTILE_UPDATED || 
                  event.type === EVENTS.PROJECTILE_EXPLODED ||
                  event.type === EVENTS.GRENADE_THROWN) {
                this.pendingProjectileEvents.push(event);
              } else if (event.type === EVENTS.WALL_DAMAGED || 
                         event.type === EVENTS.WALL_DESTROYED) {
