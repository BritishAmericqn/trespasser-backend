...
class WeaponSystem {
    ...
    checkWallHit(start, direction, maxDistance, wall) {
        ...
        const tMin = Math.max((wallBounds.left - start.x) / direction.x, (wallBounds.top - start.y) / direction.y);
        const tMax = Math.min((wallBounds.right - start.x) / direction.x, (wallBounds.bottom - start.y) / direction.y);
