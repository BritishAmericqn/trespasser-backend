...
class VisionSystem {
    ...
    constructor() {
        ...
        const pixelCount = constants_1.GAME_CONFIG.GAME_WIDTH * constants_1.GAME_CONFIG.GAME_HEIGHT;
    ...
    calculateVision(player, walls, wallsUpdated = false) {
        ...
        this.calculateMainVision(player.transform.position, player.transform.rotation, walls, vision.visiblePixels);
        // 2. Peripheral vision (30px radius, excluding 90° behind)
        this.calculatePeripheralVision(player.transform.position, player.transform.rotation, walls, vision.visiblePixels);
        // 3. Extended vision in mouse direction
        this.calculateExtendedVision(player.transform.position, player.transform.rotation, walls, vision.visiblePixels);
        // Cache the result
    ...
    isCacheValid(player, cached) {
        const timeDiff = Date.now() - cached.lastCalculated;
        if (timeDiff > 200)
            return false; // 200ms max cache (was 100ms)
    ...
    calculateMainVision(origin, centerAngle, walls, visiblePixels) {
        ...
        while (stack.length > 0) {
            ...
            if (y < 0 || y >= constants_1.GAME_CONFIG.GAME_HEIGHT)
                continue;
            ...
            while (currentX <= segment.x2) {
                ...
                for (let x = currentX - 1; x >= 0; x--) {
                    if (!this.inMainFOV(x, y, x0, y0, leftAngle, rightAngle, constants_1.GAME_CONFIG.VISION_RANGE)) {
                        break;
                    }
                ...
                for (let x = currentX; x < constants_1.GAME_CONFIG.GAME_WIDTH; x++) {
                    if (!this.inMainFOV(x, y, x0, y0, leftAngle, rightAngle, constants_1.GAME_CONFIG.VISION_RANGE)) {
                        break;
                    }
                    if (this.isWallBlocking(x, y, walls)) {
    ...
    calculatePeripheralVision(origin, centerAngle, walls, visiblePixels) {
        const x0 = Math.floor(origin.x);
        const y0 = Math.floor(origin.y);
        const radius = 30;
        ...
        for (let dy = -radius; dy <= radius; dy++) {
            for (let dx = -radius; dx <= radius; dx++) {
                ...
                const y = y0 + dy;
                // Check bounds
                if (x < 0 || x >= constants_1.GAME_CONFIG.GAME_WIDTH ||
                    y < 0 || y >= constants_1.GAME_CONFIG.GAME_HEIGHT)
                    continue;
                // Check distance
                ...
                if (this.inBlindSpot(angle, blindAngleStart, blindAngleEnd))
                    continue;
    ...
    calculateExtendedVision(origin, mouseAngle, walls, visiblePixels) {
        const x0 = Math.floor(origin.x);
        const y0 = Math.floor(origin.y);
        const extendedRange = constants_1.GAME_CONFIG.VISION_RANGE * 1.3;
    ...
    inMainFOV(x, y, x0, y0, leftAngle, rightAngle, maxRange) {
        const dx = x - x0;
        const dy = y - y0;
        const distSq = dx * dx + dy * dy;
        // Range check
    ...
    inBlindSpot(angle, blindStart, blindEnd) {
    ...
    hasLineOfSight(x0, y0, x1, y1, walls) {
    ...
    setVisible(x, y, visiblePixels) {
        const index = y * constants_1.GAME_CONFIG.GAME_WIDTH + x;
        const byteIndex = Math.floor(index / 8);
        const bitIndex = index % 8;
        // Check if already visible
        if (this.scanlineBuffer[byteIndex] & (1 << bitIndex))
            return;
        // Mark as visible
    ...
    checkVisiblePlayers(vision, players) {
        vision.visiblePlayers.clear();
        for (const [playerId, player] of players) {
            if (playerId === vision.playerId)
                continue; // Skip self
